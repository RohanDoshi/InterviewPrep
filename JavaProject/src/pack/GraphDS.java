...
//		directedGraph.depthFirstSearch();
		...
		cycleGraph.depthFirstSearch();
		Graph clone = cycleGraph.clone();
...
class Graph {
	...
	private Map<Vertex, Vertex> mapClone = new HashMap<>();
	public Graph clone() { 
	...
	private void cloneGraph(Graph g, Vertex v) {
		v.color = Color.Gray;
		Vertex cloneV = null;
		if(!mapClone.containsKey(v)) {
			cloneV = new Vertex(v.c);
			mapClone.put(v, cloneV);
			g.addVertex(cloneV);
		}
		
		
		for(Vertex edge : v.edgeTo) {
			if(edge.color.equals(Color.White)) {
				cloneGraph(g, edge);
				g.addEdge(mapClone.get(v), mapClone.get(edge));
			}
		}
		v.color = Color.Black;
	}
